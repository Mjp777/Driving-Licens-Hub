...
import com.rto.security.JWTRequestFilter;
...
public class WebSecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		}).and().authorizeRequests().antMatchers("/api/admin/**").hasRole("ADMIN").antMatchers("/api/patient/**")
				.hasAnyRole("PATIENT", "ADMIN").antMatchers("/api/doctor/**").hasAnyRole("DOCTOR", "ADMIN")
				.antMatchers("/api/image/**").permitAll()
				.antMatchers("/api/register/**", "/api/auth/**", "/swagger*/**", "/v*/api-docs/**").permitAll() // enabling
																												// global
																												// access
																												// to
																												// all
